...
Ghost Comments uses VS Code's native Comments interface. comments stay anchored to code as it moves, support Markdown replies, and can be organized with colored tags from the Activity Bar.
...
- Browse discussions by tag, file, comment, and reply from the Tagged Comments sidebar or within the Comments Panel Tab.
- Recover from accidental comment-file changes with passive local backup snapshots.

## See it in action

### Create and tag a comment

![Creating and tagging a Ghost Comment](assets/marketplace/create-and-tag.gif)

### Browse existing comments

![Viewing existing Ghost Comments](assets/marketplace/view.gif)
...
> _The first comment prompts for an author name. Change it later with the **Ghost Comments: Author Name** setting. Press `⌘ ⇧ P` (macOS) or Ctrl+Shift+P (Windows/Linux), choose **Preferences: Open Settings (UI)**, then search for **Ghost Comments: Author Name**. Existing discussions keep their original authors._
...
To change the shortcut, press `⌘ ⇧ P` (macOS) or Ctrl+Shift+P (Windows/Linux), choose **Preferences: Open Keyboard Shortcuts**, search for **Ghost Comments: Create Note**, and assign another shortcut. User keybindings override the packaged default.
...
Open the Ghost Comments Activity Bar view to browse discussions by **tag → file → comment → replies**. Selecting a comment or reply opens its file and expands the native discussion. Detached discussions open the file without selecting an unsafe range.
...
Each workspace folder stores discussions in `.gc/comments.json`. Commit this file when discussions should be shared with the repository team. Ghost Comments watches it for changes from editors and Git operations. Existing `.gc/comments.json` files and their backups are moved to the new names when first opened.
...
| ------------------------------ | ---------------------------------------------------------- | ------------------- |
| `ghostComments.authorName`     | Author stored with new comments and replies                | Prompt on first use |
